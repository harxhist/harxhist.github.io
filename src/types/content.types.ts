...
/**
 * Terminal-style landing page (e.g. Matrix / jalexine-style).
 * When landing is set, the home route renders this instead of the default layout.
 */
export type Landing = {
  ...
  /** Main bio line */
  bio: string;
  /** CTA line (e.g. "always interested in working on cool projects : feel free to reach out!") */
  cta?: string;
  /** Current status (e.g. "> doing a retreat in chongqing") */
  currentStatus?: React.ReactNode;
  /** Optional section with icon, title and optional link (e.g. "listen to my sick playlists") */
  highlight?: { icon?: string; title: string; href?: string };
  /** Interests list (single string or list) */
  interests?: string;
  /** "Pure hate" or similar section title */
  dislikesTitle?: string;
  /** List of things you dislike */
  dislikes?: string;
  /** "Take a look at" links */
  primaryLinks?: Array<{ label: string; href: string }>;
  /** "My links" section label */
  linksSectionTitle?: string;
  /** Link items (e.g. insta, hire me) */
  linkItems?: Array<{ label: string; href: string }>;
  /** Optional hero/photo image (e.g. skull) shown on the landing */
  image?: string;
};
...
export interface Home extends BasePageConfig {
  /** If set, home page uses Matrix/terminal landing layout with this content */
  landing?: Landing;
